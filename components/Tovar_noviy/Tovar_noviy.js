...
const Tovar_nov = ({ cartItems, onAdd, onRemove }) => {
  const [loader, setLoader] = useState(true);
  const [tovar, setTovar] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${env}product/category/Smartfonlar?minPrice=0&maxPrice=1500000&orderType=true&page=0&size=${productSize}`
      )
      .then((res) => {
        setTovar(res?.data?.object);
        setLoader(false);
