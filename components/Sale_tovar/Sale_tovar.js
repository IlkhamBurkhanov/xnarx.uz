...
//   };
//   return (
//     <section id="skidka" className=" sm:mx-2 bg-white pl-[16px] md:pl-0 pt-10">
...
//             </div>
//           ) : (
//             <Slider {...settings} className="mx-2.5 lg:mx-8 pl-2">
//               {tovar?.map((el) => {
//                 return (
//                   <Card
//                     key={el?.id}
...
const Sale_nov = ({ cartItems, onAdd, onRemove }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${env}product/category/Smartfonlar?minPrice=10000000&maxPrice=15000000000&orderType=true&page=0&size=${productSize}`
      )
      .then((res) => {
        setTovar(res?.data?.object);
        setLoader(false);
